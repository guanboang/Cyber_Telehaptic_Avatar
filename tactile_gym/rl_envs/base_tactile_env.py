...
class BaseTactileEnv(gym.Env):
    ...
    def step(self, action):
        ...
        #scaled_actions=[0,0,0,0,0,0]



        #如果socket接收到信息，则将数组给予scaled_actions
        # #self.robot.arm.conn.recv(1024)


        if not self.robot.arm.actiondata:
            pass
        else:
            testdata = self.robot.arm.actiondata
            print('received {!r}'.format(testdata))
            scaled_actions = testdata
