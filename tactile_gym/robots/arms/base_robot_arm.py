...
#from examples.demo_push_env import send_data
import pickle
import socket
import threading
class BaseRobotArm:
    ...
    def __init__(self, pb, robot_id, rest_poses, workframe_pos, workframe_rpy, TCP_lims):
        ...
        #         if data_rec:
        #             # Print out the data
        #             data = data_rec.decode()
        #             data_list = [float(item) for item in data.split(',')]
        #             print('received {!r}'.format(data_list))
        ...
        client_thread = threading.Thread(target=handle_client, args=(self.conn,))
        client_thread.start()
    ...
    def print_joint_pos_vel(self):
        ...
        #在这里将关节角（joint_pos）通过socket发出去。

        print("joint pos: ", joint_pos)

        joint_pos_tr = [str(item) for item in joint_pos]
        joint_data=",".join(joint_pos_tr)

# 将字符串编码为字节流（默认使用UTF-8编码）
        byte_stream = joint_data.encode()

        #send_data(joint_data)
        #conn.send(joint_data.encode())
